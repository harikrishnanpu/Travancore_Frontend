...
export default function OrderBillScreen() {
  ...
  const [showroom, setshowRoom] = useState('main shop');
  ...
  const [unit, setUnit] = useState('psc'); // Only 'pcs' or 'kg' now
  ...
  const [gstRate, setGstRate] = useState(0);      // NEW
 const [expiryDate, setExpiryDate] = useState(''); // NEW
  ...
  const handleLocalSave = () => {
    const billingData = {
      ...
      totalGST: gstAmount, // Updated: Only total GST is included
      grandTotal,
      discount,
      ...
      products: products.map((product) => ({
        ...product,
        total: (product.quantity * product.sellingPriceinQty).toFixed(2),
        gstAmount: parseFloat(product.gstAmount).toFixed(2),
      })),
    };
  ...
  useEffect(() => {
    const fetchLastBill = async () => {
      ...
      try {
        ...
        const lastInvoiceNumber = parseInt(data.lastInvoice.slice(2), 10) || 0; // Extract the number part after "Travancore Backers"
        const nextInvoiceNo = "TB" + (lastInvoiceNumber + 1).toString().padStart(2, '0'); // Ensures at least two digits
  ...
  useEffect(()=>{ 
    if(selectedProduct){
      ...
      setDisplaysellingPrice(parseFloat(selectedProduct.mrp).toFixed(2));
  }
  ...
  const addProductByItemId = async (product) => {
    ...
    try {
      ...
      const parsedArea = parsedActLenght * parsedActBreadth
        setSellingPrice(parseFloat(data.mrp).toFixed(2));
        setDisplaysellingPrice(parseFloat(data.mrp).toFixed(2));
        setExpiryDate(new Date(data.expiryDate)?.toISOString().substring(0, 10));
      setFetchQuantity(data.countInStock);
      setSuggestions([]);
...
const handleAddProductWithQuantity = () => {
  ...
  const gstAmount = (parseFloat(parsedSellingPrice) * parseFloat(gstRate || 0)) / 100;

  // Add product to the list with adjusted details and GST
  const productWithDetails = {
    ...
    gstRate: parseFloat(gstRate) || 0,   // NEW
    gstAmount: gstAmount.toFixed(2),     // NEW
...
const handleEditProduct = (index, field, value) => {
  ...
  if (field === 'enteredQty') {
    ...
    product.gstAmount = ((parsedValue * product.gstRate) / 100).toFixed(2);

  } else if (field === 'gstRate') {
    // Handle changes to GST Rate
    product[field] = parsedValue;
    product.gstAmount = ((product.sellingPrice * parsedValue) / 100).toFixed(2);
  } else {
...
const calculateBillSummary = () => {
  ...
  products.forEach((product) => {
    ...
    const gstAmount = calculateGST(productTotal, product.gstRate);
  ...
  const otherCharges = parseFloat(transportation || 0) + parseFloat(unloading || 0) + parseFloat(handlingcharge || 0);
  const grandTotal = subtotal + totalGST + otherCharges - totalDiscount;
...
useEffect(() => {
  const { subtotal, totalGST, grandTotal } = calculateBillSummary();

  setAmountWithoutGST(subtotal.toFixed(2));
...
}, [products, discount, transportation, unloading, handlingcharge]);
  ...
  const handleBillingSubmit = async () => {
    ...
    const billingData = {
      ...
      products: products.map((product) => ({
        ...
        gstRate: product.gstRate,          // Ensure GST Rate is included
      })),
    };
  ...
  const generatePDF = async () => {
    ...
    try {
      const response = await axios.post('https://kkTravancore Backers-backend.vercel.app/generate-pdf', 
         formData, {
        responseType: 'blob'
      });
  ...
  function printInvoice() {
    ...
    const formData = {
      ...
      products: products.map((product) => ({
        ...
        unit: product.unit,
        gstRate: product.gstRate,          // Ensure GST Rate is included
        gstAmount: product.gstAmount,      // Ensure GST Amount is included
        size: product.size,
      })),
    }
                ...
                <table className="table-auto w-full border-collapse rounded-xl shadow-sm">
                  <thead>
                    <tr className="bg-red-500 text-white text-xs">
                      ...
                      <th className="px-2 py-1 text-center">GST Amount</th>
                        ...
                        >
                          ...
                          <td className="px-2 py-2 text-center text-xs">
                            <input
                              ...
                              onChange={(e) =>
                                handleEditProduct(index, 'gstRate', e.target.value)
                              }
                              className="w-12 text-center px-1 py-1 border rounded-md text-xs"
                            />
                          </td>
                          <td className="px-2 py-2 text-center text-xs">
                            ₹{parseFloat(product.gstAmount).toFixed(2)}
                          </td>
  ...
  />
      ...
      {showSummaryModal && (
        ...
        accounts={accounts}
          ...
          totalGST={gstAmount} // Updated: Only showing total GST
          changeRef={changeRef}
