// src/screens/SiteReportEditPage.jsx
import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { useNavigate, useParams } from 'react-router-dom';
import api from './api';

export default function SiteReportEditPage() {
  ...
  useEffect(() => {
    const fetchReportDetails = async () => {
      setLoading(true);
      try {
        const { data } = await api.get(`/api/site-report/edit/${reportId}`);
        setSiteName(data.siteName); 
        setAddress(data.address);
        setCustomerName(data.customerName);
        setCustomerContactNumber(data.customerContactNumber);
        setContractorName(data.contractorName);
        setContractorContactNumber(data.contractorContactNumber);
        ...
        setError('Failed to fetch site report details.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const submitHandler = async (e) => {
    ...
    try {
      await api.put(`/api/site-report/edit/${reportId}`, reportData);
      setSuccess(true);
      alert("succesfully updated");
      navigate('/'); // Redirect to site reports list or desired page
    } catch (err) {
      setError('Failed to update the site report.');
    }
  };
    ...
    <div className="mx-auto">
      ...
      <div className="flex max-w-4xl mx-auto items-center justify-between bg-gradient-to-l from-gray-200 via-gray-100 to-gray-50 shadow-md p-5 rounded-lg mb-4">
        <div onClick={() => navigate('/')} className="text-center cursor-pointer">
          ...
          <p className="text-gray-400 text-xs font-bold">Edit Site Report</p>
        </div>
      ...
      {success && (
        <div className="fixed top-4 left-1/2 transform -translate-x-1/2 bg-green-500 text-white px-4 py-2 rounded-lg shadow-lg">
          <p className="text-xs animate-pulse font-bold">Report updated successfully!</p>
        </div>
      )}
                    ...
                    <div>
                      ...
                      {imageError && (
                        <div className="flex justify-center">
                          <p className="text-gray-400 animate-pulse text-sm">{image}</p>
                        </div>
                      )}
                    </div>
                  ) : (
